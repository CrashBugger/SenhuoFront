...
let pathEntity3 = null
let pathEntity4 = null
...
export function onMounted(mapInstance) {
  ...
  mars3d.Util.fetchJson({ url: "http://localhost/mars3d-data/newUavPath1xxx.json" }) 
  .then(function (res) {
    initPath0(res)
  })
  ...
  mars3d.Util.fetchJson({ url: "http://localhost/mars3d-data/newUavPath2xxx.json" }) 
  .then(function (res) {
    initPath1(res)
  })
  .catch(function (error) {
  ...
  mars3d.Util.fetchJson({ url: "http://localhost/mars3d-data/newUavPath3xxx.json" }) 
  .then(function (res) {
    initPath2(res)
  })
  .catch(function (error) {
  ...
  mars3d.Util.fetchJson({ url: "http://localhost/mars3d-data/newUavPath4xx.json" }) 
  .then(function (res) {
    initPath3(res)
  })
  .catch(function (error) {
  ...
  mars3d.Util.fetchJson({ url: "http://localhost/mars3d-data/newUavPath5x.json" }) 
  .then(function (res) {
    initPath4(res)
  })
  .catch(function (error) {
...
function initPath0(data) {
  ...
  const fixedRoute = new mars3d.graphic.FixedRoute({
    ...
    speed: 310,
    positions: [
      [101.418937, 27.987174, 160],
      [101.418937, 27.987174, 160],
      [101.421316, 27.979769, 160],
      [101.413242, 27.983607, 160],
      [101.407548, 27.980040, 160],
      [101.423695, 27.972363, 160],
      [101.426074, 27.964958, 160],
      [101.401853, 27.976473, 160],
      [101.396158, 27.972906, 160],
      [101.428453, 27.957553, 160],
      [101.430832, 27.950147, 160],
      [101.390464, 27.969339, 160],
      [101.384769, 27.965772, 160],
      [101.433211, 27.942742, 160],
      [101.435590, 27.935337, 160],
      [101.379074, 27.962205, 160],
      [101.373380, 27.958638, 160]
    ],
...
function initPath3(data) {
  ...
  // 创建path对象
  pathEntity3 = new mars3d.graphic.PathEntity({ // path路径 Entity矢量数据
    position: property, // 含时序的点的集合（坐标位置）
    orientation: new Cesium.VelocityOrientationProperty(property), // 实体方向
    style: { // 样式信息
      resolution: 1, // 指定在对位置进行采样时步进的最大秒数
    ...
    popup: "大疆精灵4号"// 绑定的popup弹窗值
  })
  graphicLayer.addGraphic(pathEntity3)
  ...
  const djData_keyong = { name: "大疆精灵四号", time: null, td_jd: null, td_wd: null, td_gd: null, flytime: "80分钟", position: null }
  const point = data[0]
  djData_keyong.td_jd = Number(point.x.toFixed(6))
  djData_keyong.td_wd = Number(point.y.toFixed(6))
  djData_keyong.td_gd = point.z
...
function initPath4(data) {
  ...
  // 创建path对象
  pathEntity4 = new mars3d.graphic.PathEntity({ // path路径 Entity矢量数据
    position: property, // 含时序的点的集合（坐标位置）
    orientation: new Cesium.VelocityOrientationProperty(property), // 实体方向
    style: { // 样式信息
      resolution: 1, // 指定在对位置进行采样时步进的最大秒数
    ...
    popup: "大疆精灵5号"// 绑定的popup弹窗值
  })
  graphicLayer.addGraphic(pathEntity4)
  ...
  const djData_keyong = { name: "大疆精灵五号", time: null, td_jd: null, td_wd: null, td_gd: null, flytime: "70分钟", position: null }
  const point = data[0]
  djData_keyong.td_jd = Number(point.x.toFixed(6))
  djData_keyong.td_wd = Number(point.y.toFixed(6))
  djData_keyong.td_gd = point.z
...
function addDemoGraphic3(graphicLayer) {
  ...
  scrollWall = new mars3d.graphic.ScrollWall({
    positions: [
        ...
        [101.454886, 28.076823, 12.29],
        [101.504254, 27.953019, 11.07],
        [101.447485, 27.89831, 11.01],
        [101.3506014, 27.94437, 17.11]
    ],
    style: {
      diffHeight: 2500, // 高度
      color: "#f33349",
